...
final class AuditTracker {
    ...
    private static final ThreadLocal<Map<String, EntityState>> STORE = ThreadLocal.withInitial(HashMap::new);
    ...
    private static final ThreadLocal<Boolean> IN_DMS_UPDATE = ThreadLocal.withInitial(() -> false);
    private static final ThreadLocal<List<DeferredDMSUpdate>> DEFERRED_DMS_UPDATES = ThreadLocal
    ...
    public void track(Model entity, String[] names, Object[] state, Object[] previousState) {
        ...
        checkAndUpdateDMS(entity, names, state, previousState);
    ...
    private Object getValue(
        ...
        // If value is not in the L1 cache (may be it's cleared), it may cause lazy
     ...
     * AuditInterceptor#afterTransactionCompletion(Transaction)} method to clear the
     * change recording.
     */
    public void clear() {
        STORE.remove();
        ...
        UPDATED.remove();
        CURRENT_USER.remove();
        DEFERRED_DMS_UPDATES.remove(); // clear deferred updates
    }

    /**
     * This method should be called from {@link
     * AuditInterceptor#beforeTransactionCompletion(Transaction)} method to finish
     * change recording.
     *
    ...
    public void onComplete(Transaction tx, User user) {
        try {
            ...
            this.processDeferredDMSUpdates();
        } finally {
            clear();
            JPA.em().flush();
        }
    ...
    static class BeforeTransactionCompleteService {
        ...
        @Inject
        private Event<BeforeTransactionComplete> event;
    ...
    private void checkAndUpdateDMS(Model entity, String[] names, Object[] state, Object[] previousState) {
        // SKIP if this is a new entity (creation, not update)
        ...
        // Check if entity has relationships to any of our configured entity types (Employee, Project, etc.)
        boolean hasConfiguredRelationship = false;
        for (int i = 0; i < names.length; i++) {
            // Check if this field changed and is one of our configured relationship types
            if (!Objects.equal(state[i], previousState[i]) && state[i] != null) {
                Property prop = mapper.getProperty(names[i]);
                if (prop != null && prop.isReference() && !prop.isCollection()) {
                    // Check if this is Employee or Project field
                    String targetClass = prop.getTarget() != null ? prop.getTarget().getName() : null;
                    if ("com.axelor.apps.hr.db.Employee".equals(targetClass)
                            || "com.axelor.apps.project.db.Project".equals(targetClass)) {
                        hasConfiguredRelationship = true;
                        break;
                    }
                }
            }
        ...
        if (dmsFileProperty != null && hasConfiguredRelationship) {
            log.info("DMS Update: Relationship changed for {} (ID: {}). Scheduling DMS update after transaction.",
                    entity.getClass().getSimpleName(),
                    entity.getId());

            DEFERRED_DMS_UPDATES.get().add(
                    new DeferredDMSUpdate(entity.getId(), entity.getClass().getName()));
        }
    }

    @SuppressWarnings("all")
    private void processDeferredDMSUpdates() {
        List<DeferredDMSUpdate> updates = DEFERRED_DMS_UPDATES.get();
        if (updates.isEmpty()) {
            return;
        }

        log.info("Processing {} deferred DMS updates", updates.size());

        DMSFileRepository dmsRepo = Beans.get(DMSFileRepository.class);

        for (DeferredDMSUpdate update : updates) {
            try {
                log.info("Processing DMS update for {} (ID: {})", update.entityModel, update.entityId);

                boolean moved = dmsRepo.moveFileToCorrectHome(update.entityId, update.entityModel);

                if (moved) {
                    log.info("Successfully moved DMS file to new location");
                } else {
                    log.info("DMS file already in correct location or no file to move");
                }

            } catch (Exception e) {
                log.error("Error processing DMS update for {} ID {}",
                        update.entityModel, update.entityId, e);
            }
        }
    }
